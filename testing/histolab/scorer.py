...
class CellularityScorer(Scorer):
    ...
    def __init__(self, consider_tissue: bool = True, preprocess: Optional[Callable] = None) -> None:
...
class NucleiScorer(Scorer):
    ...
    def __init__(self, preprocess: Optional[Callable] = None) -> None:
    ...
    def __call__(self, tile: Tile) -> float:
        ...
        mask_raw_nuclei = np.array(
            tile.apply_filters(filters_raw_nuclei).image)
        mask_nuclei_clean = np.array(
            tile.apply_filters(filters_nuclei_cleaner).image)

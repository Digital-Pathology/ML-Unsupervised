...
class Tiler(Protocol):
    ...
    def locate_tiles(
        ...
        for coords, one_outline in self._tile_coords_and_outline_generator(
            ...
            draw.rectangle(tuple(rescaled),
                           outline=one_outline, width=linewidth)
        return img
...
class GridTiler(Tiler):
    ...
    def extract(
        ...
        self.pixel_overlap = int(
            self._scale_factor(slide) * self.pixel_overlap)
    ...
    def tile_size(self, tile_size_: Tuple[int, int]):
        if tile_size_[0] < 1 or tile_size_[1] < 1:
            raise ValueError(
                f"Tile size must be greater than 0 ({tile_size_})")
            ...
            for j in range(n_tiles_column):
                ...
                if self._are_coordinates_within_extraction_mask(
                    ...
                    tile_wsi_coords = scale_coordinates(
                        reference_coords=tile_lvl_coords,
                        reference_size=slide.level_dimensions(
                            level=self.level),
    ...
    def _grid_coordinates_generator(
        ...
        for region in regions:
            bbox_coordinates_thumb = region_coordinates(
                region)  # coords of the bbox
            ...
            binary_mask_region = regions_to_binary_mask(
                [region], binary_mask.shape)
...
class RandomTiler(Tiler):
    ...
    def _random_tile_coordinates(
        ...
        tile_w_thumb = (
            tile_w_lvl * binary_mask.shape[1] /
            slide.level_dimensions(self.level)[0]
        )
        tile_h_thumb = (
            tile_h_lvl * binary_mask.shape[0] /
    ...
    def _tiles_generator(
        ...
        * the maximum number of iterations ``max_iter`` is reached
        ...
        while True:
            tile_wsi_coords = self._random_tile_coordinates(
                slide, extraction_mask)
            try:
                tile = slide.extract_tile(
                    tile_wsi_coords,
...
class ScoreTiler(GridTiler):
    ...
    def _tiles_generator(
        ...
        sorted_tiles_by_score = sorted(
            all_scores, key=lambda x: x[0], reverse=True)
